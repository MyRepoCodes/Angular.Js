angular.module("app.modules.user-manager.client-manager", [
    'app.modules.user-manager.client-manager.create',
    'app.modules.user-manager.client-manager.edit',
    'app.modules.user-manager.client-manager.download'
  ])

  .config(function ($stateProvider) {
      $stateProvider
        .state('loggedIn.modules.user-manager.client-manager', {
          url: '/client-manager',
          views: {
            'manager-content': {
              templateUrl: 'modules/user-manager/client-manager/client-manager.tpl.html',
              controller: 'UserManagerClientManagerController'
            }
          }
        });
    }
  ...
  .controller('UserManagerClientManagerController', function ($scope, $state, $filter, $modal, CONFIGS, ngTableParams, clientManagers, $translate) {
    // Permission
    ...
    // Edit
    $scope.edit = function (clientManager) {
      $state.go('loggedIn.modules.user-manager.client-manager.edit', {id: clientManager.id});
    };

    // permanentlyDelete
    $scope.permanentlyDelete = function (clientManager) {
      // Open Warning
      $scope.modal = $modal.open({
        ...
        resolve: {
          message: function () {
            return 'Are you sure you want to delete this client manager?';
          }
        }
      });
      $scope.modal.result.then(function (result) {
        if (result === true) {
          clientManagers.delete(clientManager.id).then(function () {
            for (var i = 0; i < $scope.clientManagerList.length; i++) {
              if ($scope.clientManagerList[i].id === clientManager.id) {
                $scope.clientManagerList.splice(i, 1);
              }
            }
            $scope.reload();
          }, function (error) {
    ...
    // InAcitve
    $scope.remove = function (clientManager) {
      // Open Warning
      $scope.modal = $modal.open({
        controller: 'WarningController',
        templateUrl: 'modules/alert/warning.tpl.html',
      ...
      $scope.modal.result.then(function (result) {
        if (result === true) {
          clientManagers.remove(clientManager.id,{screenName: $translate.instant('auditLogs.screenName.userManager')}).then(function () {
            for (var i = 0; i < $scope.clientManagerList.length; i++) {
              if ($scope.clientManagerList[i].id === clientManager.id) {
                $scope.clientManagerList.splice(i, 1);
              }
    ...
    $scope.restore = function (participant) {
      ...
      $scope.modal.result.then(function (result) {
        if (result === true) {
          clientManagers.restore(participant.id).then(function () {
    ...
    $scope.restoreMultipleUsers = function () {
      $scope.modal = $modal.open({
        ...
        resolve: {
          userList: function () {
            return $scope.clientManagerList;
          },
          data: function () {
            return {
        ...
        if (!!userList) {
          ...
          $scope.modal.result.then(function (ok) {
            ...
            function removeUser(userList) {
              if (!!userList[0]) {
                clientManagers.restore(userList[0].id).then(function () {
                  userList.splice(0, 1);
                  removeUser(userList);
                }, function () {
    ...
    // Listening
    $scope.$on('clientManagers:list:updated', function () {
      // Update
    });
  });
