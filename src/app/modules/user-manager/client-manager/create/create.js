angular.module("app.modules.user-manager.client-manager.create", [])

.config(function($stateProvider) {
  $stateProvider
    .state('loggedIn.modules.user-manager.client-manager.create', {
      url: '/create',
      views: {
        'manager-content@loggedIn.modules.user-manager': {
          templateUrl: 'modules/user-manager/client-manager/create/create.tpl.html',
          controller: 'UserManagerClientManagerCreateController'
        }
      }
    });
})

.controller('UserManagerClientManagerCreateController', function($scope, $state, $translate, IMAGECONFIGS, STATES, ERRORCODES, utils, security, clientManagers) {
  ...
  $scope.create = function(isValid) {
    ...
    if (isValid) {
      ...
      },{screenName: $translate.instant('auditLogs.screenName.addClientManager')}).then(function(response) {
