angular.module("app.modules.user-manager.health-coach.create", [])

.config(function($stateProvider) {
  $stateProvider
    .state('loggedIn.modules.user-manager.health-coach.create', {
      url: '/create',
      views: {
        'manager-content@loggedIn.modules.user-manager': {
          templateUrl: 'modules/user-manager/health-coach/create/create.tpl.html',
          controller: 'UserManagerHealthCoachCreateController'
        }
      }
    });
})

.controller('UserManagerHealthCoachCreateController', function($scope, $state, $translate, IMAGECONFIGS, STATES, ERRORCODES, utils, security, healthCoachs) {
  ...
  $scope.create = function(isValid) {
    ...
    if (isValid) {
      ...
      },{screenName:$translate.instant('auditLogs.screenName.addHealthCoach')}).then(function(response) {
