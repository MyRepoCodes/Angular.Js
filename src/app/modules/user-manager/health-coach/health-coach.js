angular.module('app.modules.user-manager.health-coach', [
  'app.modules.user-manager.health-coach.create',
  'app.modules.user-manager.health-coach.edit',
  'app.modules.user-manager.health-coach.download'
])

.config(function ($stateProvider) {
    $stateProvider
      .state('loggedIn.modules.user-manager.health-coach', {
        url: '/health-coach',
        views: {
          'manager-content': {
            templateUrl: 'modules/user-manager/health-coach/health-coach.tpl.html',
            controller: 'UserManagerHealthCoachController'
          }
        }
      });
  }
...
.controller('UserManagerHealthCoachController', function ($scope, $state, $filter, $translate, $modal, CONFIGS, ngTableParams,security, healthCoachs) {
  ...
  $scope.tableParams = new $scope.NgTableParams({
    ...
    getData: function ($defer, params) {
      function pagination() {
        ...
        $scope.loading = true;
        healthCoachs.get(params2, headers, false).then(function (data) {
  ...
  // Edit
  $scope.edit = function (healthCoach) {
    $state.go('loggedIn.modules.user-manager.health-coach.edit', {id: healthCoach.id});
  };

  // permanentlyDelete
  $scope.permanentlyDelete = function (healthCoach) {
    // Open Warning
    $scope.modal = $modal.open({
      ...
      resolve: {
        message: function () {
          return 'Are you sure you want to delete this health coach?';
        }
      }
    });
    $scope.modal.result.then(function (result) {
      if (result === true) {
        healthCoachs.delete(healthCoach.id).then(function () {
          for (var i = 0; i < $scope.userList.length; i++) {
            if ($scope.userList[i].id === healthCoach.id) {
  ...
  // Remove
  $scope.remove = function (healthCoach) {
    // Open Warning
    $scope.modal = $modal.open({
      controller: 'WarningController',
      templateUrl: 'modules/alert/warning.tpl.html',
    ...
    $scope.modal.result.then(function (result) {
      if (result === true) {
        healthCoachs.remove(healthCoach.id,{screenName: $translate.instant('auditLogs.screenName.editHealthCoach') }).then(function () {
          for (var i = 0; i < $scope.userList.length; i++) {
            if ($scope.userList[i].id === healthCoach.id) {
              $scope.userList.splice(i, 1);
            }
  ...
  $scope.restore = function (participant) {
    ...
    $scope.modal.result.then(function (result) {
      if (result === true) {
        healthCoachs.restore(participant.id).then(function () {
      ...
      if (!!userList) {
        ...
        $scope.modal.result.then(function (ok) {
          ...
          function removeUser(userList) {
            if (!!userList[0]) {
              healthCoachs.restore(userList[0].id).then(function () {
                userList.splice(0, 1);
                removeUser(userList);
              }, function () {
  ...
  // Listening
  $scope.$on('healthCoachs:list:updated', function () {
    // Update
  });
});
