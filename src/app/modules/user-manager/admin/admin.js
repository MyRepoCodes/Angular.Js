angular.module("app.modules.user-manager.admin", [
    'app.modules.user-manager.admin.create',
    'app.modules.user-manager.admin.edit'
  ])

  .config(function ($stateProvider) {
      $stateProvider
        .state('loggedIn.modules.user-manager.admin', {
          url: '/admin',
          views: {
            'manager-content': {
              templateUrl: 'modules/user-manager/admin/admin.tpl.html',
              controller: 'UserManagerAdminController'
            }
          }
        });
    }
  ...
  .controller('UserManagerAdminController',
    function ($scope, $state, $translate, $filter, $modal, CONFIGS, ngTableParams, admins) {
      // Permission
      ...
      $scope.tableParams = new $scope.NgTableParams({
        ...
        getData: function ($defer, params) {
          function pagination() {
            ...
            params2.embed = 'portfolio';
            $scope.loading = true;
            admins.get(params2, headers, false).then(function (data) {
      ...
      $scope.edit = function (admin) {
        $state.go('loggedIn.modules.user-manager.admin.edit', {id: admin.id});
      };
      ...
      $scope.remove = function (admin) {
        ...
        $scope.modal.result.then(function (result) {
          if (result === true) {
            admins.remove(admin.id).then(function () {
          ...
          if (!!userList) {
            ...
            $scope.modal.result.then(function (ok) {
              ...
              function removeUser(userList) {
                if (!!userList[0]) {
                  admins.remove(userList[0].id).then(function () {
                    userList.splice(0, 1);
                    removeUser(userList);
                  }, function () {
      ...
      $scope.permanentlyDelete = function (admin) {
        ...
        $scope.modal.result.then(function (result) {
          if (result === true) {
            admins.delete(admin.id).then(function () {
              for (var i = 0; i < $scope.adminList.length; i++) {
                if ($scope.adminList[i].id === admin.id) {
                  $scope.adminList.splice(i, 1);
                }
      ...
      $scope.restore = function (participant) {
        ...
        $scope.modal.result.then(function (result) {
          if (result === true) {
            admins.restore(participant.id).then(function () {
              for (var i = 0; i < $scope.adminList.length; i++) {
        ...
        $scope.modal.result.then(function (userList) {
          if (!!userList) {
            ...
            $scope.modal.result.then(function (ok) {
              ...
              function removeUser(userList) {
                if (!!userList[0]) {
                  admins.restore(userList[0].id).then(function () {
                    userList.splice(0, 1);
                    removeUser(userList);
                  }, function () {
                    listError.push(userList[0]);
      ...
      // Listening
      $scope.$on('admins:list:updated', function () {
        // Update
      });
    }
  );
