angular.module("app.modules.user-manager.agent.create", [])

.config(function($stateProvider) {
  $stateProvider
    .state('loggedIn.modules.user-manager.agent.create', {
      url: '/create',
      views: {
        'manager-content@loggedIn.modules.user-manager': {
          templateUrl: 'modules/user-manager/agent/create/create.tpl.html',
          controller: 'UserManagerAgentCreateController'
        }
      }
    });
})

.controller('UserManagerAgentCreateController', function($scope, $state, $translate, IMAGECONFIGS, STATES, ERRORCODES, utils, security, agents) {
  ...
  $scope.create = function(isValid) {
    ...
    if (isValid) {
      ...
      },{screenName: $translate.instant('auditLogs.screenName.addAgent')}).then(function(response) {
