angular.module('app.modules.question.category-manager', [])

.controller('QuestionCategoryManagerController',
    function($scope, $modalInstance, CONFIGS, ngTableParams, utils, faqCategories) {
        ...
        $scope.tableParams = new $scope.NgTableParams({
            ...
            getData: function($defer, params) {
                function pagination() {
                    ...
                    $scope.loading = true;
                    faqCategories.get(params2, false).then(function(data) {
                        params.total(data.totalCount);
                        $scope.loading = false;
                        $scope.faqCategoriesList = data.faqCategoriesList;
                        $defer.resolve($scope.faqCategoriesList);
                    }, function(error) {
                        $scope.loading = false;
                    });
                }
        ...
        $scope.addCategory = function(addCategoryForm) {
            ...
            if(addCategoryForm.$valid) {
                faqCategories.post($scope.params).then(function() {
                    resetAddFrom();
                    $scope.reload();
                }, function(error) {
                });
        ...
        $scope.changeCategory = function(editCategoryForm) {
            ...
            if(editCategoryForm.$valid) {
                faqCategories.put($scope.params2).then(function() {
                    resetEditFrom();
                    $scope.reload();
                }, function(error) {
                });
        ...
        $scope.removeCategory = function(category) {
            faqCategories.remove(category.id).then(function() {
                $scope.reload();
            });
        };
