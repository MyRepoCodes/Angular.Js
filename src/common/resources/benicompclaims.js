angular.module('resources.benicompclaims', [])

.factory('benicompclaims', function ($rootScope, $q, apiService) {

  // Set base url for resource (used as API endpoint)
  var resource = 'benicompclaims';
  ...
  var service = {
    ...
    post: function (params) {
      ...
      apiService.post(resource, params).then(function (response) {
        ...
        $rootScope.$broadcast('benicompclaims:created', response);
        deferred.resolve(response);
      }, function (error) {
        error.data.status = error.status;
        deferred.reject(error.data);
    ...
    deleteMultiClaim: function (params) {
      var deferred = $q.defer();

      apiService.post(resource + '/deleteMultiClaim', params).then(function (response) {
