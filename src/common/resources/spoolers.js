angular.module('resources.spoolers', [])

  .factory('spoolers', function ($rootScope, $q, apiService, users, i18nNotifications, utils) {

    // Set base url for resource (used as API endpoint)
    var resource = 'spoolers';
    ...
    var service = {
      ...
      getWithFileName: function (params, headers, loading) {
        var deferred = $q.defer();


        apiService.get(resource + '/withFileName' + (loading === false ? '?loadingSpinnerNotShowing' : ''), params, headers).then(function (rs) {
          var response = rs.data.data;
          var data = {
            data: response.data,
            totalCount: response.totalCount
      ...
      getWithClaims: function (id, params, headers, loading) {
        var deferred = $q.defer();


        apiService.get(resource + '/' + id + '/claims' + (loading === false ? '?loadingSpinnerNotShowing' : ''), params, headers).then(function (rs) {
          var response = rs.data.data;
          var data = {
            data: response.data,
            totalCount: response.totalCount
      ...
      deleteMulti: function (params) {
        ...
        apiService.post(resource + '/inactive', params).then(function (response) {
      ...
      restoreMulti: function (params) {
        var deferred = $q.defer();

        apiService.post(resource + '/restore', params).then(function (response) {
          deferred.resolve(true);
        }, function (error) {
          error.data.status = error.status;
          deferred.reject(error.data);
